...
Command-layer path access is fenced to the active vault before file operations reach the vault backend. `src-tauri/src/commands/vault/boundary.rs` canonicalizes the configured/requested vault root, rejects `..` escapes and absolute paths outside that root, and validates writable targets through the nearest existing ancestor so note reads, saves, deletes, view-file edits, folder mutations, Git operations, MCP registration/status/snippet operations, Apple Notes imports, vault repair/guidance restores, scans, searches, and reloads cannot step outside the selected vault. If the active root itself cannot be canonicalized, the renderer treats `Active vault is not available` the same as no active vault: it clears stale vault state, drops prefetched note content, and shows the missing-vault recovery screen instead of continuing note/view requests against the disappeared path. Image attachment commands add the current vault root to the runtime asset scope after saving so files created under a previously missing `attachments/` directory can render immediately; reload and asset-scope commands validate the requested root before widening runtime asset access.
...
UI-only file actions operate on paths that are already selected or indexed in React state. Reveal-in-Finder and external-open both re-enter the active-vault command boundary through `reveal_vault_file_external` and `open_vault_file_external` before invoking the native opener, and copy-path uses the browser clipboard API. The frontend capability no longer grants unscoped opener file reveal/open-path commands; normal HTTP(S) URL opening remains available through the opener plugin. Plain-text paste reads the desktop clipboard through `read_text_from_clipboard` in Tauri so macOS WKWebView clipboard permissions do not block the command; browser/mock mode falls back to the Web Clipboard API or mock handlers. None of those actions mutate vault contents or bypass the backend write boundary.
